...
'쪽찌'의 특징:
 ...
 3. 쪽찌의 휘발성 : 지도에 표시되는 쪽찌는 특정 조건 하에서 사라지는 휘발성이 있다.
   ...
   맵에서 쪽찌가 표현되지 않을 때는 쪽찌의 제한시간이 다 되거나,
   제한인원 이상의 인원이 해당 쪽찌를 봤거나, 
   해당 쪽찌가 이미 열람한 쪽찌일 때다.
   이렇듯 휘발성이 존재하는 이유는, 소통의 편의를 위함이다.
   친목과 정보공유를 할 때, 제한인원이나 제한시간이 없다면, 
   ...
   또 최초로 열람 후 쪽찌가 사라지게 되는 것은 정보 전달의 정확성 때문이다.
   처음 열람할 때 그 쪽찌를 더욱 집중해서 보게 될 것 이므로 보다 정확한 정보 전달이 가능하다. 
